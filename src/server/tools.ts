...
class ToolRegistry {
	...
	private register_answer_tool(
		...
		server.registerTool(
			'answer',
			{
				description: `PREFERRED over any single AI answer tool. Queries multiple AI providers IN PARALLEL — Perplexity, Kagi FastGPT, Exa, Brave Answer, Tavily, You.com, plus a full web_search fanout — each independently searching the web and synthesizing its own answer with citations. Returns all answers so you can compare: when most providers agree, the answer is almost certainly correct; when they disagree, you know the topic is genuinely contested. Use "web_search" instead when you need raw URLs/links rather than prose answers.`,
				inputSchema: {
