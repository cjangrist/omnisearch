...
```bash
SESSION=$(curl -isS -X POST https://omnisearch-mcp.example.invalid/mcp \
...
curl -sS -X POST https://omnisearch-mcp.example.invalid/mcp \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json, text/event-stream' \
...
- **Worker:** `omnisearch-mcp.example.invalid` (no auth required — see `project_public_endpoint_no_auth.md` memory)
